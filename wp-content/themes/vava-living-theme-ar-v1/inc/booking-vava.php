...
function vava_booking_shared_data( int $page_id ): array {
	...
	return array_replace_recursive( vava_booking_shared_defaults(), is_array( $saved ) ? $saved : array() );
}
...
function vava_booking_service_duration_minutes( array $service, int $fallback = 60 ): int {
...
/** Resolve the service duration from VAVA Paths, using the shared fallback only when needed. */
function vava_booking_effective_duration( array $service, array $shared ): int {
	return vava_booking_service_duration_minutes( $service, max( 10, absint( $shared['default_duration'] ?? 60 ) ) );
}
...
function vava_booking_effective_working_days( array $service, array $shared ): array {
	unset( $service );
	return array_replace( array_fill_keys( array( 'sun','mon','tue','wed','thu','fri','sat' ), 0 ), (array) ( $shared['working_days'] ?? array() ) );
}
...
function vava_booking_preview_service_sample( string $lang ): array {
	...
	$defaults = 'en' === $lang ? array(
		...
		'location' => 'Online or at VAVA Center',
		'price' => '1190 SAR',
	) : array(
		...
		'location' => 'أونلاين أو في مركز VAVA',
		'price' => '1190 ر.س',
	);
	...
	return array(
		...
		'location' => trim( (string) ( $service['location'] ?? '' ) ) ?: $defaults['location'],
...
function vava_booking_payment_method_label( string $method ): string {
	return array( 'paymob' => 'Paymob', 'bank' => 'تحويل بنكي', 'cash' => 'الدفع لاحقًا' )[ $method ] ?? $method;
}
...
function vava_booking_register_post_type(): void {
	...
	register_post_type( 'vava_booking', array(
		'labels' => array(
			...
			'all_items' => 'كل الحجوزات', 'edit_item' => 'تفاصيل الحجز', 'view_item' => 'تفاصيل الحجز',
...
function vava_booking_available_slots( array $service, string $date_string, array $shared ): array {
	...
	for ( $cursor = $start; $cursor->modify( '+' . $duration . ' minutes' ) <= $end; $cursor = $cursor->modify( '+' . $interval . ' minutes' ) ) {
		...
		$slots[] = array( 'value' => $time, 'label' => wp_date( get_option( 'time_format' ) ?: 'g:i a', $cursor->getTimestamp(), $timezone ) );
	}
	return $slots;
}
...
function vava_booking_ajax_submit(): void {
	...
	$method = isset( $_POST['payment_method'] ) ? sanitize_key( wp_unslash( $_POST['payment_method'] ) ) : 'paymob';
	if ( empty( $shared['payment_methods'][ $method ] ) ) { wp_send_json_error( array( 'message' => $text['validation_error'] ), 422 ); }
...
function vava_booking_email_html( int $booking_id, string $lang = 'ar' ): string {
	...
	$time     = (string) get_post_meta( $booking_id, '_vava_booking_time', true );
	$duration = absint( get_post_meta( $booking_id, '_vava_booking_duration', true ) );
	...
	$summary = array(
		...
		array( $labels['time'], $time ),
		array( $labels['amount'], $price ),
	);
	$booking_rows = array(
		array( $labels['service'], $service, '' ),
		array( $labels['duration'], $duration . ' ' . $labels['minutes'], '' ),
...
<?php foreach ( $summary as $index => $cell ) : ?><td width="25%" valign="top" style="padding:16px 6px;text-align:center;<?php echo $index ? ( $is_en ? 'border-left:1px solid #ece6dd;' : 'border-right:1px solid #ece6dd;' ) : ''; ?>"><span style="display:block;margin-bottom:6px;font-size:10px;color:#8e887d;"><?php echo $e( $cell[0] ); ?></span><strong style="font-size:12px;color:#5d6243;"><?php echo $e( $cell[1] ); ?></strong></td><?php endforeach; ?>
</tr></table></td></tr>
...
function vava_booking_send_details_email( int $booking_id, bool $force = false, bool $send_admin = true ) {
	...
	if ( $send_admin ) {
		...
		if ( $admin_email && $admin_email !== $email ) {
			wp_mail( $admin_email, sprintf( 'VAVA Booking #%d — %s', $booking_id, (string) get_post_meta( $booking_id, '_vava_booking_service_title', true ) ), vava_booking_email_html( $booking_id, $lang ), $headers );
		}
	}
			...
			<section class="vava-booking-preview-page">
				...
				<div class="vava-booking-preview-step is-active" data-preview-pane="1">
					<div class="vava-booking-preview-service">
						<div class="vava-booking-preview-service-aside"><span><small data-preview-service-label="duration">المدة</small><b data-preview-service-value="duration"><?php echo esc_html( $service_ar['duration'] ); ?></b></span><span><small data-preview-service-label="session_type">النوع</small><b data-preview-service-value="session_type"><?php echo esc_html( $service_ar['session_type'] ); ?></b></span><span><small data-preview-service-label="location">المكان</small><b data-preview-service-value="location"><?php echo esc_html( $service_ar['location'] ); ?></b></span><span><small data-preview-service-label="price">السعر</small><b data-preview-service-value="price"><?php echo esc_html( $service_ar['price'] ); ?></b></span></div>
...
function vava_booking_render_customer_fields( array $text, string $lang ): void {
	...
	foreach ( array( 'name','email','whatsapp','previous','notes' ) as $field ) :
		...
		<article class="vava-booking-field-card is-compact">
			...
			<label class="vava-booking-check"><input name="vava_booking[<?php echo esc_attr( $lang ); ?>][fields][<?php echo esc_attr( $field ); ?>][required]" type="hidden" value="0"/><input name="vava_booking[<?php echo esc_attr( $lang ); ?>][fields][<?php echo esc_attr( $field ); ?>][required]" type="checkbox" value="1" <?php checked( ! empty( $text['fields'][ $field ]['required'] ) ); ?>/><span><?php echo 'en' === $lang ? 'Required' : 'إلزامي'; ?></span></label>
		</article>
		<?php
	endforeach;
}
...
function vava_booking_render_availability_admin_panel( array $shared ): void {
	$settings = array(
		...
		'default_duration' => array( 'ar' => 'المدة الافتراضية بالدقائق', 'en' => 'Default duration in minutes', 'type' => 'number', 'min' => '10', 'suffix_ar' => 'دقيقة', 'suffix_en' => 'min', 'placeholder_ar' => '60', 'placeholder_en' => '60' ),
			...
			<div class="vava-booking-subsection">
				...
				<div class="vava-booking-days">
					<?php foreach ( $days as $key => $label ) : ?>
						<div class="vava-booking-day">
							<label class="vava-booking-check"><input type="hidden" name="vava_booking_shared[working_days][<?php echo esc_attr( $key ); ?>]" value="0"/><input name="vava_booking_shared[working_days][<?php echo esc_attr( $key ); ?>]" type="checkbox" value="1" <?php checked( ! empty( $shared['working_days'][ $key ] ) ); ?>/><?php vava_booking_admin_i18n( $label['ar'], $label['en'], 'span' ); ?></label>
							<label><?php vava_booking_admin_i18n( 'من', 'From', 'span' ); ?><input name="vava_booking_shared[working_hours][<?php echo esc_attr( $key ); ?>][start]" type="time" value="<?php echo esc_attr( (string) $shared['working_hours'][ $key ]['start'] ); ?>"/></label>
							<label><?php vava_booking_admin_i18n( 'إلى', 'To', 'span' ); ?><input name="vava_booking_shared[working_hours][<?php echo esc_attr( $key ); ?>][end]" type="time" value="<?php echo esc_attr( (string) $shared['working_hours'][ $key ]['end'] ); ?>"/></label>
						</div>
					<?php endforeach; ?>
				</div>
			</div>
							...
							<div class="vava-booking-payment-method-body">
								...
								<?php elseif ( 'bank' === $key ) : ?>
									...
									<div class="vava-booking-admin-grid">
										...
										<label class="vava-booking-admin-field"><?php vava_booking_admin_i18n( 'IBAN', 'IBAN', 'span' ); ?><input dir="ltr" name="vava_booking_shared[bank_transfer][iban]" value="<?php echo esc_attr( (string) ( $bank['iban'] ?? '' ) ); ?>"/></label>
									...
									<p class="vava-booking-auto-note"><?php vava_booking_admin_i18n( 'تعديل IBAN أو رقم الحساب يُسجَّل في سجل أمني ويُرسل تنبيه إلى بريد إدارة الموقع.', 'Changes to the IBAN or account number are logged and an alert is sent to the site administrator.', 'span' ); ?></p>
								<?php else : ?>
...
function vava_booking_render_settings( WP_Post $post ): void {
	...
	<div class="vava-homepage-admin vava-booking-admin" data-booking-admin data-active-language="ar" data-active-section="customer" data-settings-title-ar="إعدادات صفحة الحجز" data-settings-title-en="Booking page settings">
		...
		<header class="vava-admin-toolbar vava-booking-admin-toolbar"><nav class="vava-section-tabs vava-booking-admin-tabs" aria-label="أقسام إعدادات الحجز"><button type="button" class="is-active" data-booking-tab="customer"><span>①</span><b>بيانات العميل</b></button><button type="button" data-booking-tab="availability"><span>②</span><b>المواعيد والتوافر</b></button><button type="button" data-booking-tab="payment"><span>③</span><b>طرق الدفع</b></button><button type="button" data-booking-tab="messages"><span>④</span><b>الرسائل التشغيلية</b></button></nav></header>
...
function vava_booking_admin_body_class( string $classes ): string {
	...
	if ( $screen && 'edit-vava_booking' === $screen->id ) { $classes .= ' vava-booking-admin-screen'; }
	if ( $screen && false !== strpos( (string) $screen->id, 'vava-booking-details' ) ) { $classes .= ' vava-booking-admin-screen vava-booking-details-screen'; }
	return $classes;
}
...
function vava_booking_audit_bank_settings( array $old, array $new ): void {
	...
	if ( isset( $changes['iban'] ) || isset( $changes['account_number'] ) ) {
		...
		if ( $admin_email ) {
			...
			wp_mail( $admin_email, 'VAVA — Bank account settings changed', "تم تعديل IBAN أو رقم الحساب في إعدادات الحجز.\nالمستخدم: " . ( $user->display_name ?: $user->user_login ) . "\nالتاريخ: " . current_time( 'mysql' ) );
		}
	}
}
...
function vava_booking_admin_columns( array $columns ): array {
	return array(
		'vava_booking_number' => 'رقم الحجز',
		'vava_booking_created' => 'تاريخ الإنشاء',
		'vava_booking_customer' => 'العميل',
		'vava_booking_service' => 'الخدمة',
		'vava_booking_appointment' => 'الموعد',
...
function vava_booking_allowed_admin_actions( int $booking_id ): array {
	...
	if ( in_array( $status, array( 'cancelled', 'customer_cancelled' ), true ) && vava_booking_paid_or_refunding( $booking_id ) && vava_booking_refund_remaining( $booking_id ) > 0 ) {
		$actions['refund_booking'] = array( 'label' => 'إرجاع المبلغ', 'class' => 'is-refund' );
	}
	if ( is_email( vava_booking_customer_email( $booking_id ) ) ) {
		$actions['resend_details'] = array( 'label' => 'إعادة إرسال تفاصيل الحجز', 'class' => 'is-email' );
	}
	return $actions;
}
...
function vava_booking_admin_list_url( int $open_booking = 0 ): string {
	$url = admin_url( 'edit.php?post_type=vava_booking' );
	return $open_booking ? vava_booking_admin_details_url( $open_booking ) : $url;
}
...
function vava_booking_hide_details_submenu(): void {
	remove_submenu_page( 'edit.php?post_type=vava_booking', 'vava-booking-details' );
}
add_action( 'admin_menu', 'vava_booking_hide_details_submenu', 99 );

function vava_booking_render_details_page(): void {
	if ( ! current_user_can( vava_booking_admin_capability() ) ) { wp_die( 'غير مسموح.', '', array( 'response' => 403 ) ); }
	$booking_id = isset( $_GET['booking'] ) ? absint( $_GET['booking'] ) : 0; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	if ( ! $booking_id || 'vava_booking' !== get_post_type( $booking_id ) ) { wp_die( 'الحجز غير موجود.', '', array( 'response' => 404 ) ); }
...
function vava_booking_admin_counts(): array {
	global $wpdb;
	$counts = array( 'all' => 0, 'confirmed' => 0, 'pending' => 0, 'cancelled' => 0 );
	$rows = $wpdb->get_results( $wpdb->prepare(
		"SELECT pm.meta_value AS booking_status, COUNT(DISTINCT p.ID) AS total
		FROM {$wpdb->posts} p
		LEFT JOIN {$wpdb->postmeta} pm ON pm.post_id = p.ID AND pm.meta_key = %s
		WHERE p.post_type = %s AND p.post_status = %s
		GROUP BY pm.meta_value",
		'_vava_booking_status', 'vava_booking', 'publish'
	), ARRAY_A );
...
function vava_booking_admin_dashboard_header(): void {
	...
	$counts = vava_booking_admin_counts();
	...
	$cards = array(
		'all' => array( 'label' => 'كل الحجوزات', 'caption' => 'إجمالي الحجوزات', 'icon' => 'calendar-alt' ),
		'confirmed' => array( 'label' => 'مؤكدة', 'caption' => 'حجوزات جاهزة', 'icon' => 'yes-alt' ),
		'pending' => array( 'label' => 'بانتظار التأكيد', 'caption' => 'تحتاج إجراء', 'icon' => 'clock' ),
		'cancelled' => array( 'label' => 'ملغاة أو مرفوضة', 'caption' => 'حجوزات غير نشطة', 'icon' => 'dismiss' ),
	);
	?>
	...
	<section class="vava-booking-dashboard" dir="rtl">
		<div class="vava-booking-dashboard-heading">
			<div><span>إدارة الحجوزات</span><h1>حجوزات VAVA</h1><p>إدارة جميع الحجوزات ومتابعة حالاتها وإجراءات الدفع من مكان واحد.</p></div>
			<form class="vava-booking-dashboard-search" method="get" action="<?php echo esc_url( admin_url( 'edit.php' ) ); ?>">
				...
				<label><span class="dashicons dashicons-search" aria-hidden="true"></span><input type="search" name="vava_booking_lookup" value="<?php echo esc_attr( $lookup ); ?>" placeholder="ابحث باسم العميل أو رقم الحجز" aria-label="البحث باسم العميل أو رقم الحجز"/></label>
				<button type="submit" class="button button-primary">بحث</button>
				<?php if ( '' !== $lookup ) : $clear_args = array( 'post_type' => 'vava_booking' ); if ( 'all' !== $current ) { $clear_args['vava_booking_view'] = $current; } ?><a class="button vava-booking-dashboard-search-clear" href="<?php echo esc_url( add_query_arg( $clear_args, admin_url( 'edit.php' ) ) ); ?>">مسح</a><?php endif; ?>
			</form>
		</div>
		<div class="vava-booking-stat-grid">
			<?php foreach ( $cards as $key => $card ) : ?>
			<a class="vava-booking-stat-card <?php echo $current === $key ? 'is-active' : ''; ?>" href="<?php echo esc_url( vava_booking_admin_view_url( $key ) ); ?>" data-vava-stat-card="<?php echo esc_attr( $key ); ?>">
				<span class="dashicons dashicons-<?php echo esc_attr( $card['icon'] ); ?>" aria-hidden="true"></span>
				<div><small><?php echo esc_html( $card['label'] ); ?></small><strong data-vava-count-key="<?php echo esc_attr( $key ); ?>"><?php echo esc_html( (string) ( $counts[ $key ] ?? 0 ) ); ?></strong><em><?php echo esc_html( $card['caption'] ); ?></em></div>
			</a>
			<?php endforeach; ?>
		</div>
	</section>
...
function vava_booking_admin_apply_filters( WP_Query $query ): void {
	...
	if ( isset( $group_statuses[ $view ] ) ) {
		$query->set( 'meta_query', array( array( 'key' => '_vava_booking_status', 'value' => $group_statuses[ $view ], 'compare' => 'IN' ) ) );
	}
...
function vava_booking_admin_created_parts( int $post_id ): array {
	...
	return $timestamp ? array( 'date' => wp_date( 'Y-m-d', $timestamp ), 'time' => wp_date( 'H:i', $timestamp ) ) : array( 'date' => '', 'time' => '' );
}
...
// V4R10 — floating row actions without shifting the details button.
function vava_booking_render_row_actions( int $post_id ): void {
	$actions = vava_booking_allowed_admin_actions( $post_id );
	unset( $actions['refund_booking'] );
	?>
	<div class="vava-booking-row-actions" data-booking-actions="<?php echo esc_attr( (string) $post_id ); ?>">
		<a class="vava-booking-details-link" href="<?php echo esc_url( vava_booking_admin_details_url( $post_id ) ); ?>">عرض التفاصيل</a>
		<?php if ( $actions ) : ?>
		<div class="vava-booking-action-menu">
			<button type="button" class="vava-booking-action-menu-toggle" aria-expanded="false" aria-label="إجراءات الحجز">•••</button>
			<div class="vava-booking-action-menu-panel" hidden>
				<?php foreach ( $actions as $decision => $definition ) : ?><button type="button" class="vava-booking-action-option <?php echo esc_attr( (string) ( $definition['class'] ?? '' ) ); ?>" data-vava-booking-action data-booking-id="<?php echo esc_attr( (string) $post_id ); ?>" data-decision="<?php echo esc_attr( $decision ); ?>"><?php echo esc_html( (string) ( $definition['label'] ?? '' ) ); ?></button><?php endforeach; ?>
			</div>
		</div>
		<?php endif; ?>
	</div>
...
function vava_booking_render_admin_receipt_uploader( int $booking_id ): void {
	if ( 'bank' !== get_post_meta( $booking_id, '_vava_booking_payment_method', true ) ) { return; }
	?>
	<div class="vava-booking-receipt-replace" data-booking-receipt-upload data-booking-id="<?php echo esc_attr( (string) $booking_id ); ?>">
		<div><strong>رفع / استبدال إثبات التحويل</strong><small>JPG أو PNG أو WEBP أو PDF — بحد أقصى 5MB.</small></div>
		<label><input type="file" accept=".jpg,.jpeg,.png,.webp,.pdf" data-booking-receipt-file/><span data-booking-receipt-filename>اختر ملفًا جديدًا</span></label>
		<button type="button" class="button button-primary" data-booking-receipt-submit>رفع الإيصال</button>
		<p data-booking-receipt-message role="status"></p>
	</div>
	<?php
}
...
function vava_booking_ajax_admin_replace_receipt(): void {
	...
	ob_start(); vava_booking_render_receipt_cell( $booking_id ); $receipt_html = ob_get_clean();
	ob_start(); vava_booking_render_state_cell( $booking_id ); $state_html = ob_get_clean();
	wp_send_json_success( array( 'message' => 'تم رفع إثبات التحويل وإعادة الحجز للمراجعة.', 'html' => $details_html, 'receiptHtml' => $receipt_html, 'stateHtml' => $state_html, 'bookingId' => $booking_id, 'counts' => vava_booking_admin_counts() ) );
}
add_action( 'wp_ajax_vava_booking_admin_replace_receipt', 'vava_booking_ajax_admin_replace_receipt' );
...
function vava_booking_admin_column_value( string $column, int $post_id ): void {
	...
	switch ( $column ) {
		...
		case 'vava_booking_service':
			echo '<div class="vava-booking-service-cell is-text-only"><div><strong>' . esc_html( (string) get_post_meta( $post_id, '_vava_booking_service_title', true ) ) . '</strong><small>' . esc_html( (string) get_post_meta( $post_id, '_vava_booking_service_kind', true ) ) . '</small><small>' . esc_html( absint( get_post_meta( $post_id, '_vava_booking_duration', true ) ) . ' دقيقة' ) . '</small></div></div>';
			break;
		case 'vava_booking_appointment':
			echo '<div class="vava-booking-cell vava-booking-cell--appointment"><strong class="vava-booking-ltr">' . esc_html( (string) get_post_meta( $post_id, '_vava_booking_date', true ) ) . '</strong><small class="vava-booking-ltr">' . esc_html( (string) get_post_meta( $post_id, '_vava_booking_time', true ) ) . '</small></div>';
			break;
...
function vava_booking_action_label( string $action ): string {
	$labels = array(
		...
		'note_saved' => 'حفظ ملاحظة إدارية', 'resend_details' => 'إعادة إرسال تفاصيل الحجز', 'refund_recorded' => 'تسجيل استرداد مبلغ',
	);
...
function vava_booking_render_action_history( int $booking_id ): void {
	...
	<?php foreach ( $entries as $entry ) : $user = ! empty( $entry['user_id'] ) ? get_userdata( absint( $entry['user_id'] ) ) : null; ?>
		<article><i aria-hidden="true"></i><div><strong><?php echo esc_html( (string) ( $entry['label'] ?? 'تحديث الحجز' ) ); ?></strong><small><?php echo esc_html( (string) ( $entry['time'] ?? '' ) ); ?><?php echo $user ? ' — ' . esc_html( (string) $user->display_name ) : ''; ?></small><?php if ( ! empty( $entry['note'] ) ) : ?><p><?php echo esc_html( (string) $entry['note'] ); ?></p><?php endif; ?></div><span><?php echo esc_html( vava_booking_status_label( (string) ( $entry['status'] ?? '' ) ) ); ?></span></article>
	<?php endforeach; ?>
	</div>
	<?php
}
...
function vava_booking_render_refund_overview( int $booking_id, string $status, float $remaining ): void {
	...
	<section class="vava-booking-detail-card vava-booking-refund-overview <?php echo $eligible ? 'is-eligible' : ''; ?>">
		...
		<div class="vava-booking-refund-metrics">
			<div><span>تم رده</span><strong><?php echo esc_html( number_format_i18n( $refunded_total, 2 ) ); ?></strong></div>
			<div><span>المتبقي</span><strong><?php echo esc_html( number_format_i18n( $remaining, 2 ) ); ?></strong></div>
		</div>
		<?php if ( $eligible ) : ?>
			<p>الحجز مدفوع وملغي، ويمكن تسجيل رد المبلغ كاملًا أو جزئيًا مع إثبات العملية.</p>
		...
		<?php elseif ( $paid && ! in_array( $status, array( 'cancelled', 'customer_cancelled' ), true ) ) : ?>
			<p>يصبح إجراء إرجاع المبلغ متاحًا تلقائيًا بعد إلغاء الحجز المدفوع.</p>
		<?php elseif ( $history ) : ?>
			<p>تم تسجيل عمليات استرداد لهذا الحجز.</p>
		<?php else : ?>
			<p>لا يوجد استرداد مطلوب لهذا الحجز.</p>
		<?php endif; ?>
		<?php if ( $history ) : ?><div class="vava-booking-refund-history"><?php foreach ( array_slice( $history, 0, 5 ) as $item ) : $user = ! empty( $item['user_id'] ) ? get_userdata( absint( $item['user_id'] ) ) : null; ?><article><div><strong><?php echo esc_html( number_format_i18n( (float) ( $item['amount'] ?? 0 ), 2 ) ); ?></strong><span><?php echo esc_html( (string) ( $item['method'] ?? '' ) ); ?></span></div><small><?php echo esc_html( (string) ( $item['date'] ?? $item['time'] ?? '' ) ); ?><?php echo $user ? ' — ' . esc_html( (string) $user->display_name ) : ''; ?></small><?php if ( ! empty( $item['reference'] ) ) : ?><em>مرجع: <?php echo esc_html( (string) $item['reference'] ); ?></em><?php endif; ?></article><?php endforeach; ?></div><?php endif; ?>
	</section>
	<?php
}
...
function vava_booking_render_admin_fullpage_content( int $post_id ): void {
	...
	$time             = (string) get_post_meta( $post_id, '_vava_booking_time', true );
	$duration         = absint( get_post_meta( $post_id, '_vava_booking_duration', true ) );
	...
	<div class="wrap vava-booking-details-page" dir="rtl" data-vava-booking-details-page data-booking-id="<?php echo esc_attr( (string) $post_id ); ?>">
		<div class="vava-booking-details-page-toolbar">
			<a class="vava-booking-back-link" href="<?php echo esc_url( vava_booking_admin_list_url() ); ?>"><span aria-hidden="true">→</span> العودة إلى الحجوزات</a>
		...
		<div class="vava-booking-details-page-grid" data-vava-booking-detail-content>
			...
			<main class="vava-booking-details-main">
				...
				<?php if ( $refund_remaining > 0 && isset( vava_booking_allowed_admin_actions( $post_id )['refund_booking'] ) ) : ?>
				<section class="vava-booking-refund-panel" data-vava-refund-panel hidden><header><div><small>استرداد يدوي موثق</small><h2>إرجاع المبلغ للعميل</h2></div><button type="button" data-vava-refund-close aria-label="إغلاق">×</button></header><div class="vava-booking-refund-grid"><label>نوع الاسترداد<select data-refund-type><option value="full">كامل</option><option value="partial">جزئي</option></select></label><label>المبلغ المسترد<input type="number" step="0.01" min="0.01" max="<?php echo esc_attr( (string) $refund_remaining ); ?>" value="<?php echo esc_attr( (string) $refund_remaining ); ?>" data-refund-amount></label><label>تاريخ الاسترداد<input type="date" value="<?php echo esc_attr( wp_date( 'Y-m-d' ) ); ?>" data-refund-date></label><label>طريقة إرجاع المبلغ<input type="text" placeholder="تحويل بنكي" data-refund-method></label><label>الرقم المرجعي<input type="text" data-refund-reference></label><label>إثبات الاسترداد<input type="file" accept=".jpg,.jpeg,.png,.webp,.pdf" data-refund-proof></label><label class="is-full">ملاحظة داخلية<textarea rows="3" data-refund-note></textarea></label></div><footer><span>المتبقي للاسترداد: <b><?php echo esc_html( number_format_i18n( $refund_remaining, 2 ) ); ?></b></span><button type="button" class="vava-booking-refund-submit" data-vava-booking-refund-submit data-booking-id="<?php echo esc_attr( (string) $post_id ); ?>">تسجيل إرجاع المبلغ</button></footer></section>
				<?php endif; ?>
			</main>
		</div>

		<footer class="vava-booking-details-actions"><div><?php vava_booking_render_action_buttons( $post_id, 'page' ); ?></div><a href="<?php echo esc_url( vava_booking_admin_list_url() ); ?>">العودة إلى قائمة الحجوزات</a></footer>
	</div>
	<?php
}
	...
	<article class="vava-booking-reader-article vava-booking-reader-article--workspace" dir="rtl" data-booking-id="<?php echo esc_attr( (string) $post_id ); ?>">
		...
		<div class="vava-booking-admin-workspace">
			<aside class="vava-booking-admin-summary-rail">
				<section class="vava-booking-detail-card is-accent"><h3>ملخص الحجز</h3><?php vava_booking_admin_detail_row( 'رقم الحجز', '#' . $post_id, 'ltr' ); vava_booking_admin_detail_row( 'تاريخ الحجز', $created_at ); vava_booking_admin_detail_row( 'وقت الحجز', $created_at ? wp_date( 'H:i', strtotime( $created_at ) ) : '' ); vava_booking_admin_detail_row( 'حالة الحجز', vava_booking_status_label( $status ) ); vava_booking_admin_detail_row( 'طريقة الدفع', vava_booking_payment_method_label( $method ) ); vava_booking_admin_detail_row( 'الإجمالي', $amount ); if ( $refund_status ) { vava_booking_admin_detail_row( 'حالة الاسترداد', vava_booking_refund_status_label( $refund_status ) ); } ?></section>
				<?php if ( 'bank' === $method ) : ?><section class="vava-booking-detail-card vava-booking-admin-receipt-card"><h3>إيصال التحويل</h3><?php if ( $receipt_url ) : ?><a href="<?php echo esc_url( $receipt_url ); ?>" target="_blank" rel="noopener"><?php if ( 0 === strpos( $receipt_mime, 'image/' ) ) : ?><img src="<?php echo esc_url( $receipt_url ); ?>" alt="إيصال التحويل"><?php else : ?><span class="vava-booking-refund-document">PDF</span><?php endif; ?><span>عرض الإيصال بالحجم الكامل</span></a><?php else : ?><p>لا يوجد إيصال متاح.</p><?php endif; ?><?php vava_booking_render_admin_receipt_uploader( $post_id ); ?></section><?php endif; ?>
			</aside>
			<main class="vava-booking-admin-workspace-main">
				<div class="vava-booking-admin-primary-grid">
					<section class="vava-booking-detail-card"><h3>الدفع وإثبات التحويل</h3><?php vava_booking_admin_detail_row( 'طريقة الدفع', vava_booking_payment_method_label( $method ) ); vava_booking_admin_detail_row( 'حالة الدفع', vava_booking_payment_status_label( $payment_status ) ); vava_booking_admin_detail_row( 'الإجمالي', $amount ); vava_booking_admin_detail_row( 'تاريخ الدفع', (string) get_post_meta( $post_id, '_vava_booking_paid_at', true ) ); if ( 'bank' === $method ) { vava_booking_admin_detail_row( 'اسم المحوِّل', (string) ( $transfer['transfer_name'] ?? '' ) ); vava_booking_admin_detail_row( 'البنك المحوّل منه', (string) ( $transfer['from_bank'] ?? '' ) ); vava_booking_admin_detail_row( 'مرجع العملية', (string) ( $transfer['reference'] ?? '' ), 'ltr' ); } ?></section>
				...
				<div class="vava-booking-admin-secondary-grid">
					...
					<section class="vava-booking-detail-card vava-booking-action-history"><h3>سجل النشاط</h3><?php vava_booking_render_action_history( $post_id ); ?><?php if ( $last_email ) : ?><p class="vava-booking-last-email">آخر إعادة إرسال: <?php echo esc_html( $last_email ); ?></p><?php endif; ?></section>
				</div>
				<?php if ( $refund_remaining > 0 && isset( vava_booking_allowed_admin_actions( $post_id )['refund_booking'] ) ) : ?>
				<section class="vava-booking-refund-panel" data-vava-refund-panel hidden><header><div><small>استرداد يدوي موثق</small><h3>إرجاع المبلغ للعميل</h3></div><button type="button" data-vava-refund-close>×</button></header><div class="vava-booking-refund-grid"><label>نوع الاسترداد<select data-refund-type><option value="full">كامل</option><option value="partial">جزئي</option></select></label><label>المبلغ المسترد<input type="number" step="0.01" min="0.01" max="<?php echo esc_attr( (string) $refund_remaining ); ?>" value="<?php echo esc_attr( (string) $refund_remaining ); ?>" data-refund-amount></label><label>تاريخ الاسترداد<input type="date" value="<?php echo esc_attr( wp_date( 'Y-m-d' ) ); ?>" data-refund-date></label><label>طريقة إرجاع المبلغ<input type="text" placeholder="تحويل بنكي" data-refund-method></label><label>الرقم المرجعي<input type="text" data-refund-reference></label><label>إثبات الاسترداد<input type="file" accept=".jpg,.jpeg,.png,.webp,.pdf" data-refund-proof></label><label class="is-full">ملاحظة داخلية<textarea rows="3" data-refund-note></textarea></label></div><footer><span>المتبقي للاسترداد: <b><?php echo esc_html( number_format_i18n( $refund_remaining, 2 ) ); ?></b></span><button type="button" class="button button-primary" data-vava-booking-refund-submit data-booking-id="<?php echo esc_attr( (string) $post_id ); ?>">تسجيل إرجاع المبلغ</button></footer></section>
				<?php endif; ?>
			</main>
		</div>
...
function vava_booking_process_admin_action( int $booking_id, string $decision, string $note = '' ): string {
	...
	if ( 'approve_cash' === $decision ) {
		...
		$new_status = 'confirmed'; $new_payment = 'paid'; $event = 'bank_approved'; update_post_meta( $booking_id, '_vava_booking_paid_at', current_time( 'mysql' ) );
		...
		if ( vava_booking_paid_or_refunding( $booking_id ) ) {
...
function vava_booking_ajax_admin_action(): void {
	...
	$messages = array(
		...
		'approve_bank' => 'تم اعتماد التحويل البنكي وتأكيد الحجز.',
		'reject_bank' => 'تم رفض التحويل البنكي وتحرير الموعد.',
		...
		'resend_details' => 'تمت إعادة إرسال تفاصيل الحجز للعميل.',
...
function vava_booking_ajax_admin_refund(): void {
	...
	$old_status = (string) get_post_meta( $booking_id, '_vava_booking_status', true );
	vava_booking_append_action_log( $booking_id, 'refund_recorded', $entry['note'], $old_status, $old_status, 'refund_pending', $completed ? 'refunded' : 'partially_refunded' );
	vava_booking_send_refund_update( $booking_id, $amount, $completed );
	ob_start(); vava_booking_render_admin_details_content( $booking_id, true ); $details_html = ob_get_clean();
	ob_start(); vava_booking_render_state_cell( $booking_id ); $state_html = ob_get_clean();
	ob_start(); vava_booking_render_row_actions( $booking_id ); $actions_html = ob_get_clean();
	wp_send_json_success( array( 'message' => $completed ? 'تم تسجيل رد المبلغ بالكامل.' : 'تم تسجيل الاسترداد الجزئي.', 'html' => $details_html, 'stateHtml' => $state_html, 'actionsHtml' => $actions_html, 'bookingId' => $booking_id, 'counts' => vava_booking_admin_counts() ) );
}
...
function vava_booking_notify_admin_customer_cancellation( int $booking_id, string $mode, string $reason ): void {
	...
	$lines = array(
		...
		'الموعد: ' . (string) get_post_meta( $booking_id, '_vava_booking_date', true ) . ' ' . (string) get_post_meta( $booking_id, '_vava_booking_time', true ),
	);
